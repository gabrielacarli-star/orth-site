...
import { registrarVendaAdmin, type VendaFormState } from "@/lib/actions/vendas"
...
export function NovaVendaAdminForm({
  vendedores,
}: {
  vendedores: { id: string; nome: string }[]
}) {
  const [state, formAction, pending] = useActionState(registrarVendaAdmin, initialState)
  ...
  useEffect(() => {
    if (state?.success) {
      ...
      router.push("/sistema/admin/vendas")
    }
  ...
  return (
    ...
    >
      <div className="sm:col-span-2">
        <label className="block text-sm text-orth-muted mb-1.5">Vendedor responsável</label>
        <select name="vendedor_id" required className={inputClass} defaultValue="">
        ...
        {vendedores.length === 0 && (
          <p className="text-orth-muted text-xs mt-1">
            ...
            <a href="/sistema/admin/vendedores" className="text-orth-sky hover:text-white">
